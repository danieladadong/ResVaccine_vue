<template>
  <div class="mod-config">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click="addOrUpdateHandle(scope.row.vsid)">修改</el-button>
            <el-button
              type="text"
              size="small"
              @click="deleteHandle(scope.row.vsid)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- <el-pagination
      ...
      <editpoint
        ...
        ref="PointAddOrUpdate"
        ></editpoint>
  </div>
</template>

<script>
import PointAddOrUpdate from './editpoints.vue';
...
export default {
  ...
  methods: {
    ...
    addOrUpdateHandle(id) {
      ...
      this.$nextTick(() => {
        this.$refs.PointAddOrUpdate.init(id);
      });
    },
    // 删除
    deleteHandle(id) {
      ...
      this.$confirm(
        ...
        ids.forEach(function(element){
            ...
            deletePoints(deleteData).then(resp=>{
              if (resp.status == 200) {
                that.$message({
                  message: resp.data,
                  type: "success",
